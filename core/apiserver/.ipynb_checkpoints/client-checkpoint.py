...
class Client:
    ...
    def request(self, _path, **kwargs):
        ...
        try:
            ...
            raise ApiflyError('Unable to connect to the server, please try again later.')
        finally:
            t1 = time.time()
    ...
    def prepare_headers(self):
        """Prepares headers for sending a request to the apifly server.
        """
        headers = {}
        if self.auth_token:
    ...
    def handle_response(self, response):
        if response.status_code == 200:
            ...
            raise ApiflyError("Authorization token mismatch.")
        elif response.status_code == 404:
            raise ApiflyError("Requested function not found")
        elif response.status_code == 422:
            raise ValidationError(response.json()["error"])
        elif response.status_code == 500:
            if response.headers["Content-Type"] == "application/json":
                raise ApiflyError(response.json()["error"])
            else:
                raise ApiflyError(response.text)
        else:
            raise ApiflyError("Oops! Something really bad happened")
...
def RemoteFunction(client, func_name):
    def wrapped(*args, **kwargs):
        if args:
            raise ApiflyError('Apifly functions only accept named arguments')
