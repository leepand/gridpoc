...
class ViewAlgoDetails(ViewBase):
    def GET(self,username,projectname):
        ...
        algo_tm = algo_detail_info['algo_tm']
        ...
        details_info={'infer_algo_info':python_client_info,'tags_info':tags_info,'datainfo':"|".join(datalist),'algodesc':algodesc,\
                     "algo_tm":algo_tm,"funcslist":funcs_info}
...
#增加每个service的request counts和starinfo(已存在)
#update 2019-01-30增加他、同一tag下的services
class ViewApiServList(ViewBase):
    def __init__(self):
        self._rDict = {
            "UName": {'n': "UName", 't': str, 'v': None},
            "Tag": {'n': "Tag", 't': str, 'v': None}
        }
    ...
    def _deal_serv_user_list(self):
        ...
        if self.UName=='zoo':
            if self.Tag=="NonetagQuery":
                bRet,algo_list =ArthurAlgoList.data_show(algo_uId,'zoo','NonetagQuery')
            else:
                bRet,algo_list =ArthurAlgoList.data_show(algo_uId,'zoo',self.Tag)
        else:
            if self.UName=='myself': 
                bRet,algo_list =ArthurAlgoList.data_show(algo_uId,'user','NonetagQuery','all')#service_status=仅登陆用户可以查看自己的所有服务，查看其他人的只能看到其上线的服务
            else:
                bRet,algo_list =ArthurAlgoList.data_show(algo_uId,'user','NonetagQuery')
        ...
        if len(_algo_list)<1:
            ...
            for _inter in range(len(_algo_list)):
                ...
                #add services request info
                try:
                    _bRet_,algo_detail_info= ArthurService.algo_proj_info(algouId,algoname)
                    host=algo_detail_info['host']
                    port = algo_detail_info['port']
                    ...
                    if "body" not in response:
                        response["body"]=0
                    algo_request_info[algoname]=int(response["body"])
                except:
                    algo_request_info[algoname]=0
        
        algo_list_info = {"user":self.get_user_name(),"algolist":algo_list,'star_info':algo_star_info,\
                          'algo_User_star_all':algo_User_star_all,'algo_User_algocnt':algo_User_algocnt,"algo_request_info":algo_request_info}
