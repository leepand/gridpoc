...
layui.config({
    ...
    $.get("../static/json/message.json",function(data){
        ...
        for(var i=0; i<data.length; i++){
            ...
            msgHtml += '    </div>';
            msgHtml += '  </td>';
            msgHtml += '  <td class="msg_time">'+'<small class="list-time">'+data[i].askTime+'</small>'+'</td>';
                        ...
                        if(data[i].msgId == id){
                            if(data[i].msgReply && data[i].msgReply.length != 0){
                                for(var j=0;j<data[i].msgReply.length;j++){
                                    ...
                                    msgReplyHtml += '    </div>';
                                    msgReplyHtml += '  </td>';
                                    msgReplyHtml += '  <td class="msg_time list-time">'+data[i].msgReply[j].askTime+'</td>';
                            ...
                            msgReplyHtml += '    </div>';
                            msgReplyHtml += '  </td>';
                            msgReplyHtml += '  <td class="msg_time list-time">'+data[i].askTime+'</td>';
